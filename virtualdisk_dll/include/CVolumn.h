...
public:
	...
	/************************************************************************/
	/*  @Func GetNewName
	/*					当虚拟磁盘路径包含通配符时，需产生新文件名
	/*	@Para FileName1 物理磁盘中的文件名
	/*	@Para FileName2 虚拟磁盘中的文件名
	/*	@Para NewName   最后返回的实际存储文件名
	/************************************************************************/
	int GetNewName(char FileName1[], char FileName2[], CMyString& NewName);
	...
	int SetVolumn(int nIndex = 0)	
	{
		...
		m_RootDir.SetVolumn(m_nVolumnIndex);
